...
from query import update_portfolio,delete_portfolio,load_portfolio,insert_portfolio1, get_mf_data
import uuid
        ...
        if st.button("Submit", type="primary"):
            ...
            for row in rows_data:
                 if row["type"] in ("Stock", "Gold"):
                    ...
                    mf_txns.append((
                       row["type"], row["asset"], row["txn_date"], row["txn_type"],
            ...
            #st.write(mf_txns)
            for i in mf_txns:
                type, fund, txn_date, txn_type, amout,nav, units = i
                #print(type, fund, txn_date, txn_type, amout,nav, units)
                try:
                    dat = get_mf_data(56,fund)
                    print(dat)
